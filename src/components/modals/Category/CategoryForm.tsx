...
export const CategoryForm: React.FC<CategoryFormProps> = ({
	...
	return (
		...
		>
			<div>
				<h3 className='text-2xl font-bold'>{category.id ? 'Edit category' : 'Add new category'}</h3>
				<p className='text-[var(--theme-text-muted)] text-sm'>
					{category.id ? 'Category details' : 'Create a custom category to organize your passwords'}
				</p>
			</div>
			<Input
				label='Category Name'
			...
			<div className='flex flex-col relative'>
				...
				<div>
					<Input
						...
						value={category.params.icon}
						placeholder='lucide:tag'
					/>
					<Icon
						icon={category.params.icon || 'lucide:tag'}
			...
			<div>
				...
				<div className='grid grid-cols-6 gap-1'>
					{Object.entries(availableColors).map(([color, { bg, selectedColor }]) => {
						const isSelected = category.params.color === color
						return (
							<button
								key={color}
								type='button'
			...
			<div className='grid grid-cols-5 gap-2 mt-6'>
				...
				/>
				<Button
					content={category.id ? 'Save Category' : 'Add Category'}
